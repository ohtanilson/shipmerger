...
#--------------------------#
market_num = 20
obsd_many_markets = Vector{DataFrame}(undef,market_num)
for kk = 1:market_num
	temp_threshold_tonnage = 1
	temp_subsidy_amount = 1
	temp_subsidy_type = "shared"
	@time m = carrier_mod(randomseed = kk,#1
						 ton_dist = Distributions.LogNormal(2.0,1.0),
						 N = 8,
						 β_0 = 0.0,# coefficient of covariates
						 δ_0 = 10.0,# coefficient of subsidy
						 γ_0 = 4.0,# coefficient of additional merger cost
						 ton_dim= 2)
	utility, obsd = gen_data(m,
	                threshold_tonnage =　temp_threshold_tonnage,
...
domain = [-20:2.0:40;]
res_β_0 = zeros(length(domain),market_num)
for i in 1:length(domain), kk = 1:market_num
	iter = domain[i]
	if size(obsd_many_markets[kk])[1] == m.N
	    res_β_0[i,kk], ~ = score_b(m, [iter, m.δ_0, m.γ_0], obsd_many_markets[kk],
...
sum_res_β_0 = sum(res_β_0, dims = 2)
Plots.plot(domain, sum_res_β_0,
           label="score",
...
savefig("julia_merger_figure/plot_beta_$(temp_subsidy_type)_subsidy_many_markets")

domain = [-20:2.0:40;]
res_δ_0 = zeros(length(domain),market_num)
for i in 1:length(domain), kk = 1:market_num
	iter = domain[i]
	if size(obsd_many_markets[kk])[1] == m.N
	    res_δ_0[i,kk], ~ = score_b(m, [m.β_0, iter, m.γ_0], obsd_many_markets[kk],
	                        m.N, temp_threshold_tonnage,
						    temp_subsidy_amount,
							subsidy_type = temp_subsidy_type)
	else
...
sum_res_δ_0 = sum(res_δ_0, dims = 2)
Plots.plot(domain, sum_res_δ_0, label="score",
...
savefig("julia_merger_figure/plot_delta_$(temp_subsidy_type)_subsidy_many_markets")

domain = [-20:2.0:40;]
res_γ_0 = zeros(length(domain),market_num)
for i in 1:length(domain), kk = 1:market_num
	iter = domain[i]
	if size(obsd_many_markets[kk])[1] == m.N
	res_γ_0[i,kk], ~ = score_b(m, [m.β_0, m.δ_0, iter],
	                        obsd_many_markets[kk], m.N, temp_threshold_tonnage,
							temp_subsidy_amount,
							subsidy_type = temp_subsidy_type)
	else
		res_γ_0[i,kk] = 0
...
sum_res_γ_0 = sum(res_γ_0, dims = 2)
Plots.plot(domain, sum_res_γ_0, label="score",
...
savefig("julia_merger_figure/plot_gamma_$(temp_subsidy_type)_subsidy_many_markets")
...
savefig("julia_merger_figure/contour_beta_gamma_$(temp_subsidy_type)_subsidy_many_markets")
...
savefig("julia_merger_figure/contour_gamma_delta_$(temp_subsidy_type)_subsidy_many_markets")
...
Plots.vline!([m.β_0], label="true β", linestyle = :dash)
Plots.hline!([m.δ_0], label="true δ", linestyle = :dash)
savefig("julia_merger_figure/contour_beta_delta_$(temp_subsidy_type)_subsidy_many_markets")
...
Plots.hline!([m.γ_0], label="true γ", linestyle = :dash)
Plots.vline!([m.β_0], label="true β", linestyle = :dash)
savefig("julia_merger_figure/contour_beta_gamma_no_merger_$(temp_subsidy_type)_subsidy_many_markets")
...
@time for ii = 1:length(N_list)
	...
	Plots.vline!([m.β_0], label="true β", linestyle = :dash)
	N_iter = Int(N_iter)
	savefig("julia_merger_figure/contour_beta_gamma_N_$(N_iter)_$(temp_subsidy_type)_subsidy_many_markets")
end
